...
{
  if (lists.size() > 0) {
    std::cout << m_filename << ": Not all sections closed in Writer" << std::endl;
  }
  if (out_owned)
  {
...
{
  if (lists.size() == 0) {
    std::cout << m_filename << ": Trying to close list '" << listname << "', which is not open" << std::endl;
    return;
  }
  if (lists.back() != listname) {
    std::cout << m_filename << ": trying to close list '" << listname << "' while list '" << lists.back() << "' is open" << std::endl;
    return;
  }
  lists.pop_back();
